            ...
            try {
                ...
                if (mek.key && mek.key.remoteJid === 'status@broadcast') {
                    ...
                    if (config.AUTO_STATUS_REACT === "true") {
                        const ravlike = sock.decodeJid(sock.user.id);
                        const statusEmojis = ['❤️', '💸', '😇', '🍂', '💥', '💯', '🔥', '💫', '💎', '💗', '🤍', '🖤', '👀', '🙌', '🙆', '🚩', '🥰', '💐', '😎', '🤎', '✅', '🫀', '🧡', '😁', '😄', '🌸', '🕊️', '🌷', '⛅', '🌟', '🗿', '🇵🇰', '💜', '💙', '🌝', '🖤', '💚'];
                        const randomEmoji = statusEmojis[Math.floor(Math.random() * statusEmojis.length)];
                        await sock.sendMessage(mek.key.remoteJid, { react: { text: randomEmoji, key: mek.key } }, { statusJidList: [mek.key.participant, ravlike] });
                    }
                    if (config.AUTO_STATUS_REPLY === "true") {
                        const user = mek.key.participant;
                        const text = config.AUTO_STATUS_MSG;
                        await sock.sendMessage(user, { text: text, react: { text: '💜', key: mek.key } }, { quoted: mek });
                    }
                }
                ...
                if (!mek.key.fromMe && config.AUTO_REACT && mek.message && mek.key.remoteJid !== 'status@broadcast') {
